...
\subsection{Hypothesis test and confidence intervals for $\ddcq$-values
            by the statistical delta method}
...
For the purposes of this paper, we do not persue this furher and restrict ourselves to case of a balance experimental design where $\eta$ is obtain relatively straight-forwardly.
...
The basic idea of bootstrapping is that inference on the $\ddcq$-value can be can conducted by resampling the sample data and performing inference on the re-sampled data.
...
\subsubsection{CIC study}

[Skriv noget mere om microarray-studiet, referencer!!??]
By microarrays sceenings of multiple myeloma, the genes MGST1 and MMSET have been detected to have a differential expression between cell lines high and low in so-called culture initiating cells (CIC).
...
In Fig.~\ref{fig:simstudy} the performance of the FPR and TPR is shown for sample size equal to 4 and 8 in each case and control group and the number of dilution steps varying between 4 and 8.
