...
\begin{equation*}
  \alpha C_q = \bigl(\log_2 F_{C_q} - \log_2 \kappa \sigma N_0 \bigr).
\end{equation*}
In order to estimate the relative amount of transcripts between case
and control (ctrl) samples for the target (tgt) gene template, we assume
...
By microarrays sceenings of multiple myeloma, the genes MGST1 and MMSET was detected to have a differential expression between cell lines high and low in so-called culture initiating cells (CIC).
