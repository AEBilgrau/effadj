...
\cite{Nordgaard2006} studied error propagation primarily on the $C_q$-values including the effect of the AE uncertainty.
...
Furthermore, they considered AE estimation from the amplification curve (thus for each well) and not from separate dilution experiments.
...
While this assumption has been contested and alternatives by branching processes suggested \cite{Peccoud1996, Jacob1998, Lalam2004, Lalam2007}, the problem \emph{still} exist as AE estimates from individual amplification curves \emph{also} have an associated error which affect all `down-steam' estimated quantities and inference.
...
Tellinghuisen and Spiess \cite{Tellinghuisen2014a, Tellinghuisen2014b, Tellinghuisen2015} stressed and discussed the importance and negative impact of improper error handling, include AE estimation, with an emphasis on determining the florescence level at the hypothetical cycle zero and $C_q$-values using different methods.

The work by \cite{Svec2015} also recently assessed the impact of the AE uncertainty as a function of the number of technical replications at each concentration and the qPCR instrument.
...
Nonetheless, they do not address the impact of the AE uncertainty on formal statistical inference on the $\ddcq$, as this paper intends.
...
The estimation of the expected $C_{q,ij}$-values and $\alpha_i$ can then be estimated simultaneously when formulated as a LMM \cite{Pinheiro2000a};
\begin{equation}
  \label{eq:model}
...
Re-sampling methods in qPCR data has been proposed by \cite{Pfaffl2002} also to infer the relative expression directly by permutation testing.
Unlike the permutation testing of \cite{Pfaffl2002}, the bootstrap is here used to estimate the only mean and standard error of $\ddcq$ and not \emph{directly} test the stated hypothesis.
The bootstrap approach also allows for constructing confidence intervals
...
These constant sample and experimental variations corresponds a homoscedastic errors on the log-scale.
...
In all instances, the mean of the bootstrap distribution agree well with the estimates obtained using the delta or Monte Carlo methods while the bootstrap scheme seems to provide a larger standard error.
...
% Main message
The commonly used efficiency corrected $\ddcq$ and many other to approaches to the analysis of qPCR data disregards the uncertainty of the estimated AE leading to increased false positive rates.
...
We stress that the problem is not limited to the $\ddcq$ statistic, but all statistics that depend on the AE.
Likewise, the problem is also persists for `single-curve' AEs estimated for each well on the plate.
...
They highlight that some methods achieve very good performance as measured by the low standard errors by tacitly assuming the AE known and fixed to 2 resulting in an apparent.
This is an unsurprising consequence as it essentially the same as disregarding the AE uncertainty.
