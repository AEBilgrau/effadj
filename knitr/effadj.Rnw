...
% Title of paper
\title{Variance approximation of efficiency corrected gene expression values in qPCR experiments}
...
\begin{abstract}
{Accurate estimation of the amplification efficiency (AE) is an important prerequisite in quantitative real-time polymerase chain reaction (qPCR).
...
implicitly assuming an infinitely precise estimate of the AE.
Determining the AE with such high precision, however, require tedious laboratory work and vast amounts of biological material.%, an often scarce resource in e.g.~molecular cancer biology.
...
As qPCR frequently is used for validation it should be a high priority to estimate and account for the uncertainty of the AE estimate to properly bound the type I error rate and achieve the desired significance level.
We provide methods founded in a linear mixed effects model (LMM) to obtain variance approximations of the efficiency adjusted $\ddcq$-value based on statistical modelling in combination with either the statistical delta method, Monte Carlo, or bootstrapping.
The LMM can used to obtain more reliable statistical inference and power calculations and enables simultaneous estimation of the AE and $\ddcq$.
We validate recent findings which suggests that MGST1 is differentially expressed between multiple myeloma cell lines with high and low abundance of culture initiating cells and that miRNA-127 is differentially expressed between testicular and nodal diffuse large B-cell lymphoma.
}
% Keywords
...
The double difference accounts for unwanted variations using one or more endogenous reference genes assumed to be approximately unchanged due to the treatment.
...
If the AE is poorly determined this underestimation can drastically increase the standard error of $\ddcq$ thereby decreasing the $p$-values and increasing the type I error probability.
...
The estimation of the expected $C_{q,i,j}$-values and $\alpha_i$ can however be estimated simultaneously when formulated as a LMM \citep{Pinheiro2000a};
\begin{equation}
  \label{eq:model}
...
$\mu_{i,j} = \alpha_i^{-1}(\log_2 F_{C_q} - \log_2 \kappa \sigma_j N_{0,i,j})$.
The random effects $A_{j/s}$ are ${\cal{N}}(0,\sigma_S^2)$-distributed and
the error terms $\epsilon_{i,j,s,k}$ are independent and ${\cal{N}}(0,\sigma^2)$-distributed.
...
One way is to simulate a large number $N$ of parameters $\vtheta_1, \ldots, \vtheta_N$ from a multivariate normal distribution using the esimated parameters ${\cal{N}}_6(\hat{\vtheta}, \hat{\vec{\Sigma}})$ and computing the sample variance of $c(\vtheta_1), \ldots, c(\vtheta_N)$.
This is closely related to the (parametric) boostrap discussed in the following section.
...
Both maximum likelihood (ML) and restricted maximum likelihood estimation (REML) of LMMs have been implemented by the R-package \texttt{lme4} based on the \texttt{nlme} package of  \cite{Pinheiro2000a}.
...
This corresponds to leaving out the terms in \eqref{eq:model} involving these parameters and thus ignoring the dilution data.
If $\gamma_\tgt^{-1} = \gamma_\rref^{-1} = 1$ is assumed, we later refer to the obtained estimate as the na\"{i}ve LMM.
...
The $\ddcq$ estimate where the uncertainty of the AE is considered is refered to as efficiency corrected and variance adjusted by either the delta method (EC\&VA1) or Monte Carlo integraion (EC\&VA2).
...
Here, both new random effects and noise terms are realized and added to the fitted parameters.
...
The difference in gene expression assessed by the $\ddcq$-values for the target genes MGST1 and MMSNET and each reference genes GAPDH and ACTB were estimated using 7 different methods:
\begin{enumerate}
  ...
  \item EC\&VA1: Efficiency corrected and variance adjusted LMM estimate using 1.~order approximation.
  \item EC\&VA2: Efficiency corrected and variance adjusted LMM estimate using Monte Carlo integration.
  \item Bootstrap: Estimate by the bootstrap described in Section \ref{sec:bootstrap} fitting the LMM and computing the EC estimate above.
  \item Par.~boostr.: Estimate by parametric bootstrapping as mentioned in Section \ref{sec:bootstrap} refitting the LMM and computing the EC estimate above.
\end{enumerate}
...
Both the variance integration by Monte Carlo (EC \& VA2) and the parametric bootstrap fail here as the some efficiency is often realized to values near zero making \eqref{eq:ddcq} very large.
...
Analogously to the previous section, the differences in gene expressions assessed by the $\ddcq$-values for the target genes mir127 and mir143 and each reference rnu6b and rnu24 gene as well the combination of the reference genes, were estimated using the 7 different methods. Again $\Sexpr{n.boots}$ bootstrapped samples were used. The results are seen in Table~\ref{table:tesits}.

Like before, the the effect estimates for unpaired t-test and LMM without efficiency correction are exactly equal however with a lower standard error for LMM.
...
Our primary approach based on the statistical delta-method to approximate the variance of efficiency adjusted $\ddcq$-values, shows that it is possible to draw statistical inference about qPCR experiments whilst more properly accounting for the AE uncertainty.
