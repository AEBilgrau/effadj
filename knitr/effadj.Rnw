...
% Tel: +45 000 0000000; Fax: +45 000 0000000;
Email: abilgrau@math.aau.dk}}

\address{%
$^{1}$Deptartment of Haematology
Aalborg University Hospital,
Sdr.\ Skovvej 15, 9000 Aalborg, Denmark,
and
$^{2}$Department of Mathematical Sciences,
Aalborg University,
Fredrik Bajers Vej 7G, 9220 Aalborg {\O}, Denmark}
...
Despite being an aging technique, real-time quantitative polymerase chain reaction (qPCR) ---``one of the most significant biotech discoveries in our time''---is still heavily used in molecular biology \citep{Rainbow1996}.
...
Here qPCR is the gold standard validation tool for the most promising gene trascripts generated by high-throughput screening studies such as microarrays or sequencing.
...
We aim to formulate a mathematical model which allows formal statistical inference about qPCR experiments with non-efficient amplifications.
...
Simultaneous estimation of the uncertainty of the efficiency amplification estimate by linear mixed effects models (LMM), which allows a more appropriate handling of the technical and sample errors, is described.
...
Others have used mixed effects modeling to identify candidate normalizing genes \cite{andersen2004,abruzzo2005}.
Oher applied the related generalized estimation equations to handle intra and inter group variation \cite{fu2006}.
However, our idea of LMMs combined with the statistical delta method, Monte Carlo integration, or bootstrapping to handle uncertainty stemming from the efficiency estimation seems to be novel and provides a general statistical framework for qPCR experiments and may be considered an extension of the strategy by \cite{Yuan2008}.
...
Lastly, in a simulation study, we show that the three methods control the false postive rate better.
...
{\small{}%
\begin{align*}%
  \log\Bigl(\frac{N_{ 0,\tgt,\case}}{N_{0,\tgt,\ctrl}}\Bigr)
  &= \log \kappa \sigma_{ \case} N_{0,\tgt,\case}
    -\log \kappa \sigma_{ \case} N_{0,\rref,\case} \\
  &\quad
       - \log \kappa \sigma_{\ctrl} N_{0,\tgt,\ctrl}
       + \log \kappa \sigma_{\ctrl} N_{0,\rref,\ctrl}  \\
  &=   - \bigl\{(\alpha_{\tgt}  C_{ q,\tgt,\case}
       -  \alpha_{\rref} C_{q,\rref,\case}) \\
  &\quad - (\alpha_{\tgt}  C_{q,\tgt,\ctrl}
...
For simplicity in this paper, we refrained from using multiple reference genes simultaneously in the $\ddcq$ estimation although our framework easily extends to this case.
...
\begin{equation}
  ...
  \label{eq:tstat}
\end{equation}
It is known that $t$ is approximately
$t$-distributed with $\eta$ degrees of freedom \cite[Section 2.4.2]{Pinheiro2000a}.
...
\subsubsection{Introduction}
In order to illustrate the effect of applying variance approximations in a data set with a limited number of dilution steps and samples we considered the arabidopsis thaliana data set published by \cite{Yuan2008}. The data set contains one gene of interest, \textit{MT7}, potentially differentially expressed under two growth conditions of the plant arabidopsis thaliana and two reference genes ubiquitin (\textit{UBQ}) and tublin.


\subsubsection{Sample and data preparation}
The arabidopsis thaliana plant growth, RNA extraction, and qPCR experiments were carried out as described in \cite{Yang2006}. The cDNA was diluted into 1-to-4 and 1-to-16 serial dilutions. Real-time PCR experiments was performed in duplicates for each concentration \citep{Yuan2008}.
...
When using variance adjustment there is no longer a highly statistical significant difference in \textit{MT7} expression between case and ctrl (two growth conditions).

The results may be surprising at first sight when considering the relatively small standard errors of the slopes in the simple linear regressions shown in Fig.~\ref{fig:cqYuan}. One might imagine that the uncertainty of the AE is negligible and thus perform the usual analysis.
...
All statistical analysis were done with R v\Sexpr{paste0(version$major, ".", version$minor)} \citep{Rproj} using \texttt{lme4}.
...
\end{center}
\caption{
  Overview of A.~thaliana experiment data.
