...
We validate recent findings suggesting that MGST1 is differentially expressed between myeloma cell lines with high and low abundance of culture initiating cells and that miRNA-127 is differentially expressed between testicular and nodal lymphomas.
Lastly, the methods are benchmarked in a simulation study.
}
% Keywords
...
$k\in\{1,\dots,K\}$ for dilution step.
...
LMMs provide a more correct quantification of the sources of variation and thereby a more correct evaluation of the uncertainty of $\mu_{i,j}$ and their derived quantities.
...
The $p$-values for the null hypotehsis of $\delta = 0$ is computed by $2\min(\hat{F}(0), 1 - \hat{F}(0))$,
...
The difference in gene expression assessed by the $\ddcq$ for the target genes MGST1 and MMSNET and each reference genes GAPDH and ACTB were estimated using 7 different methods:
\begin{enumerate}
...
Analogously to the previous section, the differences in gene expressions assessed by the $\ddcq$ for the target genes mir127 and mir143 and each reference rnu6b and rnu24 gene as well the combination of the reference genes, were estimated using the 7 different methods. Again $\Sexpr{n.boots}$ bootstrapped samples were used. The results are seen in Table~\ref{table:tesits}.
...
% Conclusions
Despite the success of qPCR much more statistical methodological research is needed, in our opinion, in order to establish qPCR even more firmly as a gold standard and reliable quantitative method to assess the abundances of nucleic acids.
