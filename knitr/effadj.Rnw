...
The methods and (in some cases drastic) impact of the AE uncertainty are illustrated in three qPCR applications and an simulation study.
...
We conclude, that the commonly used efficiency corrected quantities disregard the uncertainty of the AE leading to increased false positive rates.
...
Despite being an aging technique, real-time quantitative polymerase chain reaction (qPCR) \citep{Rainbow1996}---arguably one of the most significant biotech discoveries of all time---is still heavily used in molecular biology.
...
In life sciences, qPCR is typically applied to quantify candidate gene transcripts that are biomarkers of diagnostic, prognostic, and even predictive value of e.g.\ infectious diseases, cancer, and genetic abnormalities.
...
Laboratory guidelines and increasingly sophisticated statistical modelling have been established to combat many of these systematic errors \citep{Bustin2010, Bustin2009, Spiess2015, Ruijter2013, Rao2013}.
...
In turn, the $\ddcq$ has been modified to an AE corrected versions \citep{Pfaffl2001, Rao2013b}.
...
Although efficiency calibration has been extensively treated by \cite{Pfaffl2001} or in the more generalized model by \cite{Yuan2008}, there seems to lack a systematic studies of the unavoidable influence of the uncertainty of the AE estimate on the conclusions of qPCR experiments based on formal statistical inference.
...
However, similar problems exists as the AE estimates from the amplification curves \emph{also} have an associated standard error which affect the `down-steam' quantities and inference.
...
They fail, however, to gauge the effect of the number of concentrations used---an arguably more important variable than the number of replicates as additional technical replicates rarely contribute with much information to determine the AE.
...
And we propose and benchmark different off-the-shelf and novel resolutions to this problem.
...
First, the problem exist for \emph{all} statistical models and methods which incorrectly disregard the uncertainty of the AE estimate and is not limited to LMMs.
Second, the problem exists not only for $\ddcq$-values, but also all similar quantities (e.g.\ $\Delta C_q$) and even $C_q$ and the statistical inference based on these.
...
\citep{Steibel2009} and \citep{Matz2013} use the mixed models primarily for the $C_q$ value estimation but also derived values.
...
In a simulation study, we show that the methods proposed indeed control the false postive rate better than the conventional approach and provide further insight into the problem.
...
We study the problematic aspects of ignoring the uncertainty of AE estimate in the context of the following LMM.
Note, however, that this problem persists for \emph{all} statistical models and methods which na\"{i}ve `plug-in' the AE estimate from dilution curves or otherwise into formulae concerning the $\ddcq$ (and similar quantities).
...
Here, $k$ denote the dilution step and $x_k$ the number of 2-fold dilutions (e.g.\ $x_1 = 1$ means the first dilution step halves the original concentration).
...
Lastly, we note that algorithm \citep{Mx300P2013} we used for threshold selection and $C_q$ value extraction in the CIC and DLBCL studies may not be optimal, cf.~\citep{Ruijter2013}, as it can be affected by the AE.
Nonetheless, this has no bearing on the stated problem of this paper.
The estimated standard error of $\ddcq$ is still affected in a similar manner by the uncertainty of the AE and thus optimistic.
...
\section*{Author's contributions}
  ...
  AP: Conduced DLBCL experiment and provided data.
  MK: Conduced CIC experiment and provided data.
...
\end{center}
\caption{
  \csentence{Overview of \citet{Yuan2008} data.}
