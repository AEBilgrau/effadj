...
For each dataset the $\ddcq$ is calculated and confidence intervals and $p$-values can be obtained from the bootstrap distribution.
...
First, the standard error of $\ddcq$ is calculated by the sample standard deviation of the boostrap distribution.
