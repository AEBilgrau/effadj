...
This is probably also why the reason dilution curves, when carefully used, are still very popular in qPCR experiments.
...
A way to check this is to assess whether the dilution curves are reasonably linear, as non-constant behaviour up to the $C_q$'th cycle in our dynamic range would imply the dilution curves to level off.
...
We also note, as seen in this paper, that a low standard error in itself is not always proper benchmark of procedures.
