...
\citet{Tellinghuisen2014a, Tellinghuisen2014b, Tellinghuisen2015} stressed and discussed the importance and negative impact of improper error handling, include AE estimation, with an emphasis on determining the florescence level at the hypothetical cycle zero and $C_q$-values using different methods.

\citet{Svec2015} also recently assessed the impact of the AE uncertainty as a function of the number of technical replications at each concentration and the qPCR instrument.
