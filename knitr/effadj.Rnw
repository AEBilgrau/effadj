...
While this assumption has been contested \citet{},
the problem still exist as AE estimates from amplification curves \emph{also} have an associated error which affect all `down-steam' estimated quantities and inference.
The numerous AEs estimated for \emph{all} wells arguably increases the problem as even more errors---one for each well---is propagated futher on.
