...
Cells are defined to be culture initiating if they can initiate a sustained production of cells when cultured in vitro. The viability potential of culture initiating cells (CIC) is measured by the limiting number of cells necessary to establish an in vitro culture. This number is typically estimated by limiting dilution assays and Poisson regression [xx].
...
The bootstrap again provides a standard deviation similar to the delta method and monte carlo integraion for both miR-127 and miR-143.
