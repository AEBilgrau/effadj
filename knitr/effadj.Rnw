...
% Title of paper
\title{Variance approximation efficiency corrected gene expression values in qPCR experiments}
...
\begin{abstract}
{Accurate estimation of the amplification efficiency (AE) is an important prerequisite in real-time quantitative polymerase chain reaction (qPCR) e.g.~for quantification of gene expression.
The most commonly used correction strategy is to estimate the AE by dilution experiments and use this as a plug-in when efficiency correcting the $\ddcq$-value.
Currently it is recommended to determine the AE with a high precision as the plug-in approach do not account for its uncertainty,
corresponding to a implicit assumption of an infinitely precise estimate of the AE.
It is clear, however, that determining the AE with such high precision require tedious laboratory work and vast amounts of biological material which is often a scarce resource in e.g.\ molecular studies of cancer biology.
Violation of the assumption often leads to overly optimistic estimates of the standard error of $\ddcq$, confidence intervals, and $p$-values.
Ultimately, the type I error probability is increased.
As qPCR is often used for validation it should be a high priority to estimate and account for the accuracy of the AE estimate to properly bound the type I error rate and achieve the desired significance level.
We provide novel methods to obtain variance approximations of the efficiency adjusted $\ddcq$-value based on statistical modelling in combination with either the statistical delta method to bootstrapping.
These procedures can be utilized to obtain more reliable statistical inference based on qPCR data and power calculations.
The methods are founded in a linear mixed effects model that enables the simultaneous estimation of the AE and $\ddcq$ and provides an approximation of the variance the efficiency corrected $\ddcq$ estimate.
We validate two recent findings which suggests that MGST1 is differentially expressed between multiple myeloma cell lines with high and low numbers of culture initiating cells and that miRNA-127 is differentially expressed between testicular and nodal diffuse large B-cell lymphoma (DLBCL).
}
% Keywords
...
Here qPCR is used as a validation tool for the most promising genes generated by high-throughput screening studies such as microarrays or next generation RNA and DNA sequencing.
...
The regular $\ddcq$ method is commonly estimated by a normalized relative expression of a target gene of interest between treated (case) and untreated samples (control).
The double difference is accounting for unwanted variations using one or more endogenous reference genes assumed to be approximately unchanged due to the treatment.
...
This assumption implies a systematic underestimation of the standard error of $\ddcq$ leading to too narrow confidence intervals and decreased $p$-values under inference for $\ddcq$.
...
In this paper we describe the estimation of the uncertainty of the efficiency amplification estimate by mixed effects models, which also allows for a more appropriate handling of the technical and sample errors by either the statistical delta method or bootstrapping.

The idea of using linear mixed models (LMM) for qPCR experiments is not completely new.
...
However, our idea of LMMs combined with the statistical delta method or bootstrapping to handle uncertainty stemming from the efficiency estimation seems to be novel and provides general statistical framework for qPCR experiments and an extension of \cite{Yuan2008}.
...
The approach is used to demonstrate that multiple myeloma cancer cell lines differentially express the MGST1 gene depending on the frequency of culture initiating cells.
...
$k\in\{1,\dots,K\}$ for dilution step; and $l\in\{1,\dots,L\}$ for technical replicate.
...
\subsection[Inference for DDCq by the statistical delta method]{Inference for $\ddcq$ by the statistical delta method}

We first consider hypothesis testing and confidence intervals for $\ddcq$-values by the statistical delta method.
...
We now consider hypothesis testing and confidence intervals for $\ddcq$-values by bootstrapping as an alternative approach.

A simple alternative to the statistical delta method in assessing the $\ddcq$ estimate is the bootstrap which avoids calculating gradients and is often cited to perform better in small sample situations.

The basic idea of bootstrapping is that inference on the $\ddcq$-value can be can conducted by re-sampling the sample data and performing inference on the re-sampled data.
...
Hence a bootstrapped data set consists of the re-sampled matched pairs and the bootstrapped standard curve.
For each data set the $\ddcq$-value is calculated and confidence intervals and $p$-values can be obtained from the bootstrap distribution.

Alternatively, parametric bootstrap could be made by simulating datasets from the fitted model.
Here, both new random effects and noise terms are realized by the fitted parameters.

In the analysis of the CIC and Testis data we draw \Sexpr{n.boots} bootstrap samples (both parametric and non-parametric) and used them to carry out statistical inference.
...
We applied the described approaches to two qPCR validation experiments in two different studies of multiple myeloma (MM) and diffuse large B-cell lymphoma (DLBCL).
...
In order to gauge the performance of the methods we subsequently a simulation study.
...
By a microarray screening of multiple myeloma, the genes MGST1 and MMSET have been detected to have a differential expression between cell lines with high and low abundance of so-called culture initiating cells (CIC).
CICs are of particular interest in cancer research as they exhibit a high cellular resilience and ability to initiate repopulation.
Such properties provide clues about the molecular drivers of cancer stem-cells and their aggressiveness.
The purpose of this experiment was to validate the findings of MGST1 and MMSET.

[Skriv noget mere om microarray-studiet, referencer!!??]
...
Prompted by the research of \citet{Robertus2009} as well as our own work,
we wished to validate or falsify the differential expression of two specific so-called micro RNAs (miR) between testicular and nodal DLBCL cases.
Micro RNAs are non-coding RNA transcripts with regulatory capabilities.
\citet{Robertus2009} investigated in 50 cases of DLBCL the expression of 15 different miRs between two different DLBCL subtypes.
In this experiment, we wished to validate or falsify the differential expression of miR-127 and miR-143.

[Bedre introduktion!?, referencer!!??]
...
In our setting, the TPR of the statistical test is the probability that the test will correctly declare a result statistically significant given an non-zero effect size $\delta = c(\theta)$ between case and controls.
...
A straightforward way to obtain an estimate of the TPR is to simulate a large number $n$ of datasets under the alternative hypothesis of $c(\theta) = \delta$, fit the LMM for each dataset, and compute $t$-values $t_1, \dots, t_n$.
...
The differences in gene expressions assessed by the $\ddcq$-values for the target genes MGST1 and MMSNET and each reference GAPDH and ACTB gene as well the combination of the reference genes, were estimated using four different methods:
Paired $t$-tests and regular $\ddcq$ estimates without efficiency correction, efficiency corrected estimates with and without a variance correction by the statistical delta-method, and the mean of $1{,}000$ bootstrapped samples.
The paired $t$-tests and $\ddcq$-estimate without efficiency correction completely ignores the dilution data.
These results are seen in Table~\ref{table:cic}.

Consider the first section of Table~\ref{table:cic} regarding the target MGST1 against the reference GAPDH.
We notice the paired t-test and LMM without efficiency correction that the estimates are equal, are expected due to the balanced design, but the standard error is lower for the LMM due to improved efficiency in case-ctrl set-up.
The tests for a vanishing $\ddcq$ both
...
For all combinations of MGST1 and the reference genes, the estimate is significantly different from zero and the $5\%$ significance level.
...
Hence no good evidence for differential expression of MMSET between cell lines of high and low abundance of CICs.
...
Analogously to the previous section, the differences in gene expressions assessed by the $\ddcq$-values for the target genes mir127 and mir143 and each reference rnu6b and rnu24 gene as well the combination of the reference genes, were estimated using four different methods: Paired $t$-tests and regular $\ddcq$ estimates without efficiency correction, efficiency corrected estimates with and without a variance correction by the statistical delta-method, and the mean of $1{,}000$ bootstrapped samples.
These results are seen in Table~\ref{table:tesits}.

Like before, the paired t-test and LMM without efficiency correction that the estimates are exactly equal with a lower standard error for LMM.
...
\subsection{Simulation example and study}

First, we present results of a simulation example for a two-sided test for the null hypothesis of a vanishing $\ddcq$-value at $5\%$ significance level.
...
Each of the discussed methods were applied to the $2\times \Sexpr{n.sims}$ datasets and the $p$-value testing the null hypothesis was computed.
The results of these tests are summarized in \ref{table:cic} from which the FPR and TPR can be computed at the 0.05 cutoff.
Considering first the FPR of the test procedures.
From the table, we see that the FPR is \Sexpr{ex.fpr} for the LMM, the efficiency corrected LMM (EC), the efficient corrected with variance adjustment LLM (EC\&VA), and the bootstrap method, respectively.
...
While better, the variance adjusted estimator still do not control the FPR at $5\%$.
By construction, the variance adjusted will always perform at least as good as the EC in terms of FPR.
Perhaps surprisingly, the bootstrap has the worst performance in terms of FPR.

Secondary, TPR is computed to be \Sexpr{ex.tpr} for methods \Sexpr{names(ex.tpr)}, respectively.
As known and expected, the TPR is closely connected to the FPR where more conservative approach yield a lower TPR.
The better FPR comes a the cost of a lower true positive rate.

Terms of FPR and TPR are shown for sample sizes 4 and 8 in case and control groups in combination with the number of dilution steps varying between 4 and 8.
...
Our primary approach based on the statistical delta-method to approximate the variance of efficiency adjusted $\ddcq$-values, shows that it is possible to draw statistical inference about qPCR experiments whilst properly accounting for the AE uncertainty.
...
variance approximation was introduced, potentially leading to a change of significance for the presented data set depending on the desired significance level.
...
% Conclusions
Despite the recent success of qPCR much more statistical methodological research is needed, in our opinion, in order to establish qPCR even more firmly as a reliable quantitative method to assess the amount of nucleic acids.
