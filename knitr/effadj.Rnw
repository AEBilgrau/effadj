...
Hence the bootstrapped data set consists of the re-sampled matched pairs and the bootstrapped standard curve.
...
In the analysis of the CIC and Testis data we draw 1000 bootstrap sample and used them to carry out statistical inference.
...
The $p$-value is zero, probably due to the resolution provided by 1000 bootstrapped samples for the MGST1 gene and slightly higher for MMSET gene.
...
We simulated 1000 datasets under both the null and alternative hypothesis with 6 samples in each case and control groups and 6 dilution steps.
...
Each of the discussed methods were applied to the $2\times 1000$ datasets and the $p$-value testing the null hypothesis was computed.
