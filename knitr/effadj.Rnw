...
By microarrays screenings of multiple myeloma, the genes MGST1 and MMSET have been detected to have a differential expression between cell lines high and low in so-called culture initiating cells (CIC).
...
Such properties that provide clues about the molecular drivers of cancer stem-cells and their aggressiveness.
...
we wished to validate or falsify the differential expression of so-called micro RNAs (miRNA or miR), which are short non-coding RNAs, between testicular and nodal DLBCL cases.
\citet{Robertus2009} investigated 50 cases of DLBCL the expression of 15 different miRNAs in is differentially expressed between different DLBCL subtypes.
...
\end{equation*}
A straightforward way to obtain an estimate of the TPR is to simulate a number of $t$-values $t_1,\dots,t_n$ under the alternative hypothesis of $c(\theta) = \delta$ and then estimate the TPR by
\begin{equation*}
  \widehat{\mathrm{TPR}} =
...
Based on the above statistical model, we computed the FPR and the TPR for each discussed method under some chosen sample sizes, sample and experimental variations, and true effect sizes.
...
% Main message
Many current statistical approaches to analysis qPCR data disregard the uncertainty of the estimated amplification efficiency leading to increased false positive rates.
