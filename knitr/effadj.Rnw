...
We note that the division of $\gamma_j$ in \eqref{eq:c} is theoretically problematic as $\hat{\gamma}_j$ is normally distributed.
In practice, this is only problematic if the standard error of $\hat{\gamma}_j$ is sufficently large.
Some solve this prolem using the $\log_2$ concentration as the response and the $C_q$-values as the explanatory variables in regression of the standard curve to estiamte $\alpha_j$ directly.
This appoarch is however also not without theoretical problems as this puts the errors on the expanatory variables.
...
The bootstrap which avoids calculating gradients and is often cited to perform better in small sample situations.

The basic idea of the bootstrap is that inference of $\ddcq$ can be can conducted by re-sampling the sample data and performing inference on the re-sampled data.
...
CICs are of particular interest in cancer research as it is believed that cells with high CIC potential have stem cell like properties making them invisible to towards chemotherapy [xx].

In a search for genes associated to a high culture initiating potential in multiple myeloma (MM) we made limiting dilution experiments of of 14 MM cell lines and divided them into 7 cell lines with low and 7 cell lines with high culture initiating potential. Gene expressions were profiled by Affymetrix HG-U133 plus 2 microarrays and by the \texttt{limma} R-package [xx] we identified the genes MGST1 and MMSET to be differentially expressed between between cell lines with high and low abundance of so-called culture initiating cells (CIC).
...
\subsubsection{Introduction}
The association between oncogenesis and micro RNAs (miRNAs), short non-coding RNA transcripts with regulatory capabilities, has recently prompted an immense research activity [xx]. The possibility to change treatment strategies by transfecting complementary oligos to abnormally up-regulated miRNAs in malignant tissue is of particular interest [xx]. In that respect up-regulated miR-127 and miR-143 in testicular Diffuse Large B-cell Lymphoma (DLBCL) has shown treatment changing potential \citet{Robertus2009}. However, as the number of screened miRNAs was high and the sample size was low in Robertus et al.'s work invoking high risk false discoveries we set out to validate the differential expression of miR-127 and miR-143 in tissues from our own laboratory using our improved qPCR analysis workflow.
...
\subsubsection{Introduction}
In order to illustrate the effect of applying variance approximations in a data set with a limited number of dilution steps we considered the Arabidopsis thaliana data set published by Yuan et al. \cite{Yuan2008}. The data set contains one gene of interest, MT7, potentially differentially expressed under two growth conditions and two reference genes ubiquitin (UBQ) and tublin.


\subsection{Sample and data preparation}
The Arabidopsis thaliana plant growth, RNA extraction, and qPCR experiments were carried out as described in [xx]. The cDNA was diluted into one to four and one to sixteen serial dilutions. Real-time PCR experiments were carried out with duplication for each concentration \cite{Yuan2008}.
