...
Error propagation and the effect of the AE uncertainty on the $C_q$-values has been studied by \cite{Nordgaard2006}.
...
Furthermore, they considered AE estimation from the amplification curve (thus for each sample) and not from separate dilution experiments.
...
The work by \cite{Svec2015} recently assessed the impact of the AE uncertainty as a function of the number of technical replications at each concentration and the qPCR instrument.
...
Lastly, we note that the algorithm \cite{Mx300P2013} we used for threshold selection and $C_q$-value extraction in the CIC and DLBCL studies may not be optimal, cf.~\cite{Ruijter2013} and improvements by \cite{Spiess2015}, as it can be affected by the AE.
