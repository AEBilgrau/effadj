...
Some solve this problem using the $\log_2$ concentration as the response and the $C_q$-values as the explanatory variables in regression of the standard curve to estimate $\alpha_j$ directly.
This approach is however also not without theoretical problems as this puts the errors on the explanatory variables.
